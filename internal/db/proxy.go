...
func (d *DB) CreateProxyRule(siteDomain, ruleType, config string) (int64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("create proxy rule: %w", err)
	}
...
func (d *DB) ListProxyRules(siteDomain string) ([]ProxyRule, error) {
	...
	for rows.Next() {
		var r ProxyRule
		if err := rows.Scan(&r.ID, &r.SiteDomain, &r.RuleType, &r.Config, &r.CreatedAt); err != nil {
			return nil, err
		}
...
func (d *DB) DeleteProxyRuleForSite(id int64, siteDomain string) error {
	...
	if n == 0 {
		return fmt.Errorf("proxy rule %d not found", id)
	}
	return nil
}
